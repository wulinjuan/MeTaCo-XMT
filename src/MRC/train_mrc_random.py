...
from tensorize_mrc_x import QaDataProcessor
...
class QaRunner:
    def __init__(self, config_name, gpu_id=0, seed=None, saved_suffix=None, model_path=None, sub_name='', config_file_path="mrc_experiments.conf", cl_model=False, squad_mrc_similarity=False):
    ...
    def initialize_model(self, saved_suffix=None, model_path=None):
        ...
        if model_path:
            ...
            path_ckpt = join(f'/data/home10b/wlj/code/SSDM_MRC-master/data/MRC_data/{file_name}/', f'model_{model_path}/pytorch_model.bin')
    ...
    def train(self, lang='en'):
        ...
        # Set up data
        train_dataset = self.data.get_meta(lang=lang, only_dataset=True, data_name='x-metra-sample', train_dataset=self.config.train_dataset)
        dev_examples, dev_features, dev_dataset = self.data.get_source(conf['train_dataset'], 'dev.txt', lang=lang, model_path=self.model_path_squad)
        ...
        file_name = self.optimizer_model_path.split('_seed')[0]
        path_ckpt = join(f'/data/home10b/wlj/code/SSDM_MRC-master/data/MRC_data/{file_name}/', f'model_{self.optimizer_model_path}/optimizer.pt')
...
def main(ml_type, lang='en', sub_name='', config_file_path = "mrc_experiments.conf", xquad_test=True, mlqa_test=True, tydiqa_test=True, squad_mrc_similarity=False):
    ...
    for seed in seed_list:
        ...
        if xquad_test:
            dataset_name = "xquad"
            evaluator = cross_lingual_mrc_test.Evaluator_meta(config_name, saved_suffix, gpu_id, seed=runner.seed, sub_name=sub_name,
        ...
        if mlqa_test:
            dataset_name = "mlqa"
            evaluator = cross_lingual_mrc_test.Evaluator_meta(config_name, saved_suffix, gpu_id, seed=runner.seed, sub_name=sub_name, config_file_path=config_file_path)
...
if __name__ == '__main__':
    ...
    tydiqa_test = False
    lang = sys.argv[1]
    main(ml_type, lang=lang, sub_name=sub_name, config_file_path=config_file_path,
         xquad_test=xquad_test, mlqa_test=mlqa_test, tydiqa_test=tydiqa_test,
         squad_mrc_similarity=True)