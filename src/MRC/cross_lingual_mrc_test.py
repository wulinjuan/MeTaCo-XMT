...
from train_mrc import QaRunner
import os
import json
...
class Evaluator:
    def __init__(self, config_name, saved_suffix, gpu_id, seed):
        self.saved_suffix = saved_suffix
        self.runner = QaRunner(config_name, gpu_id, seed=seed, saved_suffix=saved_suffix)
    ...
    def evaluate_task(self, dataset_name):
        ...
        print(all)
        return all_f1, all_em


class Evaluator_meta:
    def __init__(self, config_name, saved_suffix, gpu_id, seed=None, sub_name='', config_file_path="mrc_experiments.conf"):
        self.saved_suffix = saved_suffix
        if seed:
            self.runner = QaRunner(config_name, gpu_id, seed=seed, sub_name=sub_name, config_file_path=config_file_path,
                                   saved_suffix=saved_suffix, squad_mrc_similarity=True)
        else:
            self.runner = QaRunner(config_name, gpu_id, sub_name=sub_name, config_file_path=config_file_path,
                                   saved_suffix=saved_suffix, squad_mrc_similarity=True)
        # self.model = self.runner.initialize_model(saved_suffix)
    ...
    def evaluate_task(self, dataset_name):
        ...
        all_f1, all_em = [], []
        for lang in langs[dataset_name]:
            examples, features, dataset = self.runner.data.get_source(dataset_name, 'test', lang)
            result_path = self.get_output_result_file(dataset_name, lang)
            ...
            if os.path.exists(result_path):
                ...
                metrics, _ = self.runner.evaluate(
                    examples, features, dataset, step=0, tb_writer=None,
                    output_results_file=self.get_output_result_file(dataset_name, lang),
                    output_prediction_file=self.get_output_prediction_file(dataset_name, lang),
                    output_nbest_file=self.get_output_nbest_file(dataset_name, lang),
                    output_null_log_odds_file=self.get_output_null_log_odds_file(dataset_name, lang),
