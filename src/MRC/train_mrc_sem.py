...
from contrastive_learning.model import LatentModel
...
class QaRunner:
    def __init__(self, config_name, gpu_id=0, seed=None, saved_suffix=None, model_path=None, sub_name='',
        ...
        if cl_model:
            self.cl_model = LatentModel(self.config, seed=self.seed)
            self.cl_model.load_state_dict(torch.load(self.config.path_ckpt, map_location=torch.device('cpu')),
                                          strict=False)
        else:
            self.cl_model = None
    ...
    def initialize_model(self, saved_suffix=None, model_path=None):
        ...
        if model_path:
            ...
            path_ckpt = join(f'/data/home10b/wlj/code/SSDM_MRC-master/data/MRC_data/{file_name}/',
    ...
    def train(self, lang='en'):
        ...
        # Set up data
        train_dataset = self.data.get_meta(lang=lang, only_dataset=True, data_name='x-metra',
        ...
        for epo in range(epochs):
            ...
            for batch in train_dataloader:
                ...
                maml = l2l.algorithms.MAML(self.model, lr=3e-6, first_order=True)
                # loss_qry_all = 0.0
                ...
                # Evaluate
                if step == 0 or step % conf['eval_frequency'] == 0:
...
def main(ml_type, lang='en', sub_name='', config_file_path="mrc_experiments.conf", xquad_test=True, mlqa_test=True,
         tydiqa_test=True, squad_mrc_similarity=False):
