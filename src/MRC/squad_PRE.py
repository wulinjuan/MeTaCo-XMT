...
def squad_convert_example_to_features(
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        encoded_dict = tokenizer.encode_plus(
            ...
            stride=max_seq_length - doc_stride - len(truncated_query) - sequence_pair_added_tokens,  # Overlapping/encoding length
...
def squad_convert_example_to_features_tydi(example, max_seq_length, doc_stride, max_query_length, is_training, lang2id=None):
    features = []
    if is_training and not example.is_impossible:
        # Get start and end position
        start_position = example.start_position
...
def squad_convert_examples_to_features_tydi(
    examples,
    tokenizer,
    max_seq_length,
    doc_stride,
    ...
    with Pool(threads, initializer=squad_convert_example_to_features_init, initargs=(tokenizer, config)) as p:
        annotate_ = partial(
            squad_convert_example_to_features_tydi,
            max_seq_length=max_seq_length,
            doc_stride=doc_stride,
            max_query_length=max_query_length,
            is_training=is_training,
