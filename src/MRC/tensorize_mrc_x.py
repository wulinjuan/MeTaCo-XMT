...
class QaDataProcessor:
    ...
    def get_meta(self, lang='en', only_dataset=False, data_name='x-metra', train_dataset='tydiqa'):
        load_dir = os.path.join(f"../../data/qa/sim_datasets", data_name)  #
        load_dir_ = os.path.join(f"../../data/qa/sim_datasets", 'x-metra')
        ## Meta-training
        with open(os.path.join(load_dir_, 'spt_examples.pkl'), 'rb') as f:
            spt_examples = pickle.load(f)

        with open(os.path.join(load_dir, lang + '_qry_examples.pkl'), 'rb') as f:
            qry_examples = pickle.load(f)

        spt_features, qry_features, dataset = squad_convert_examples_to_meta_features(spt_examples, qry_examples,
                                                                                      self.get_tokenizer(),
                                                                                      max_seq_length=self.max_seg_len,
                                                                                      doc_stride=self.doc_stride,
                                                                                      max_query_length=self.max_query_len,
        ...
        return dataset if only_dataset else (spt_features, qry_features, dataset)
    ...
    def get_source(self, dataset_name, partition, lang='en', only_dataset=False, cl_model=None, model_path=None):
        ...
        if dataset_name == 'squad':
            ...
            data_dir = join(self.config['download_dir'], dataset_name, f'MLQA_V1/test/')
            ...
            data_file = f'tydiqa.{lang}.{partition}.json'
    ...
    def get_cache_feature_path(self, dataset_name, partition, lang):
        ...
        cache_name = f'{partition}.{lang}.{self.max_seg_len}.{self.config.meta_example_num}'
