...
import pickle
import logging
from squad_PRE import SquadProcessor, squad_convert_examples_to_features
import util
...
class QaDataProcessor:
    ...
    def get_source(self, dataset_name, partition, only_dataset=False, lang="en"):
        ...
        if dataset_name == 'squad':
            ...
            data_dir = f'/data/home10b/wlj/code/Meta_MRC/data/mlqa/MLQA_V1/dev/'
            data_file = f'dev-context-{lang}-question-{lang}.json'
        else:
    ...
    def get_dev_source(self, dataset_name, partition, only_dataset=False, lang="en"):
        assert dataset_name in ['squad', 'tydiqa', 'mlqa']
        if dataset_name == 'squad':
            data_dir = join(self.config['download_dir'], dataset_name)
