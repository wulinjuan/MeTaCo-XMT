...
class L1DistanceLoss(nn.Module):
  ...
  def forward(self, predictions, label_batch, length_batch, device):
    ...
    total_sents = length_batch
    squared_lengths = length_batch*length_batch
    if total_sents > 0:
...
class L1DepthLoss(nn.Module):
  ...
  def forward(self, predictions, label_batch, length_batch, device):
    ...
    """
    total_sents = length_batch
    labels_1s = (label_batch != -1).float().to(device)
    predictions_masked = predictions * labels_1s
    labels_masked = label_batch * labels_1s
    if total_sents > 0:
