...
from Syntax_distance_model import data_utils

import numpy as np

from Syntax_distance_model_ablation.config import get_base_parser
...
class evaluator:
    ...
    def evaluate(self, all_data, func_name, verbose=False):
        ...
        s_score = []
        preds = []
        if 'zh_dev' in all_data.keys():
            y = "zh_dev"
            data1 = all_data[y]['1'][0]
            data2 = all_data[y]['1'][1]
            golds = all_data[y]['1'][2]
        else:
            y = 'zh'
            data1 = all_data[y]['1'][0]
            data2 = all_data[y]['1'][1]
