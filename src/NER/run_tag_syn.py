...
sys.path.append("/data/home10b/wlj/code/Meta_MRC/")
from src.Syntax_distance_model_ablation import train_helper
from src.Syntax_distance_model_ablation.models import prob_wmd

sys.path.append("../src/NER_Meta")
...
def build_support_features(base_features, target_features=None, support_size=2, cl_model=None):
    ...
    if cl_model is not None:
        m = torch.load(cl_model, map_location=torch.device('cpu'))
        args = m['config']
        args.gpu_id = 0
        e = train_helper.experiment(args, args.save_file_path)
        cl_model = prob_wmd(experiment=e)
        cl_model.load_state_dict(m["state_dict"], strict=False)
        support_size = 64
...
def load_and_cache_examples_meta(args, tokenizer, labels, pad_token_label_id, mode, lang,
                                 lang2id=None, few_shot=-1, sbert=False, ner_model=None, cl_model=None):
    # Make sure only the first process in distributed training process
    # the dataset, and the others will use the cache
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()
    ...
    # Load data features from cache or dataset file
    cached_features_file = os.path.join(args.data_dir, "cached_{}_{}_{}_{}".format(mode, "en_bert",
                                                                                   list(filter(None,
                                                                                               args.model_name_or_path.split(
                                                                                                   "/"))).pop(),
                                                                                   str(args.max_seq_length)))
    ...
    cached_features_file = os.path.join(args.data_dir, "cached_{}_{}_{}_{}_{}".format(mode, lang, "norm",
                                                                                          list(filter(None,
                                                                                                      args.model_name_or_path.split(
                                                                                                          "/"))).pop(),
                                                                                          str(args.max_seq_length)))

    if os.path.exists(cached_features_file) and cl_model is not None:
        logger.info("Loading features from cached file %s", cached_features_file)
        qry_features = torch.load(cached_features_file)
    else:
        qry_features = build_support_features(spt_features, target_features=features, support_size=2, cl_model=cl_model)
        if cl_model is not None:
            if args.local_rank in [-1, 0]:
                logger.info(
                    "Saving features into cached file {}, len(features)={}".format(cached_features_file,
                                                                                   len(qry_features)))
                torch.save(qry_features, cached_features_file)
    ...
    for _ in tqdm(range(args.batch_sz)):
        ...
        for index in qry_indices:
            sent = qry_features[index].sentence
            if '演唱人' in qry_features[index].sentence:
                print('yes')
            if qry_features[index].sentence =='Saat ini ia bermain untuk PSIS Semarang .':
                print('yes')
            # '湖北省潜江市
            if qry_features[index].sentence =='湖北省潜江市':
                print('yes')
            # '在台灣，黑田長禮的鳥類研究調查改變了歐美學者的舊分類，更對帝雉等台灣獨特珍貴鳥類留下豐盛的文獻研究。'

            if qry_features[index].sentence == '在台灣，黑田長禮的鳥類研究調查改變了歐美學者的舊分類，更對帝雉等台灣獨特珍貴鳥類留下豐盛的文獻研究。':
                print('yes')
            # '蔣中正#(##中華民國總統、中國國民黨總裁#)'

            if qry_features[index].sentence == "2015年，東國大學在QS世界大学排名亚洲排第77。其中以出產明星藝人的'''戲劇电影學系'''而聞名。":
                print('yes')
            # "'''佐洛乌伊洛克'''（匈牙利语：），是匈牙利佐洛州所辖的一个村，总面积9.64平方公里，总人口106，人口密度11.0人/平方公里（2010年1月1日）。"

            if qry_features[index].sentence == "'''佐洛乌伊洛克'''（匈牙利语：），是匈牙利佐洛州所辖的一个村，总面积9.64平方公里，总人口106，人口密度11.0人/平方公里（2010年1月1日）。":
                print('yes')
            ...
            sentence = spt_features[spt_indices[0]].sentence
            ids = spt_features[spt_indices[0]].label_ids
...
def main():
    ...
    parser.add_argument("--output_dir", default='../../output/NER/output_mbert_syn/', type=str,
    ...
    parser.add_argument("--q_qry", default=6, type=int)
    parser.add_argument("--meta_example_num", default=1, type=int)
    parser.add_argument("--meta_steps", default=3, type=int)
    parser.add_argument("--batch_sz", default=600, type=int)
    parser.add_argument("--batch_size", default=4, type=int)
    parser.add_argument("--gpu_id", default=0, type=int)
    parser.add_argument("--meta_num", default=500, type=int)
    parser.add_argument("--cl_model_path",
                        default='../../output/sdmm/mbert_norm/best.ckpt',
                        type=str)
    ...
    parser.add_argument("--train_langs", default="id", type=str,
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples_meta(args, tokenizer, labels, pad_token_label_id, mode="train",
                                                     ...
                                                     sbert=False, ner_model=None, cl_model=args.cl_model_path)
    ...
    # Prediction
    all_f1 = ""
    f = open('result_norm.txt', 'a+', encoding='utf-8')
